...
python3 "$script_dir/wiki_check.py" "$wiki_dir"

temp_root=$(mktemp -d "${RUNNER_TEMP:-/tmp}/pxaudit-wiki.XXXXXX")
...
if ! git -c "http.extraheader=AUTHORIZATION: basic ${auth_header}" \
  clone --quiet "$wiki_url" "$wiki_repo"; then
  echo "unable to clone $wiki_url; enable the GitHub Wiki and create its first page if it is empty" >&2
  exit 1
fi
