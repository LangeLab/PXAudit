...
Safe 3 to 64 character identifiers from other repositories are accepted, including MassIVE (`MSV`), jPOST (`JPST`), and iProX (`IPX`) identifiers. They score `Unverifiable` because PXAudit 0.6.0 only queries PRIDE.

The first public package release, v0.6.0, remains PRIDE-only. Partner adapters are planned after publication.
